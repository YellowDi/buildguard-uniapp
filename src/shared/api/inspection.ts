...
function resolveTaskDetail(raw: TaskDetailRaw): TaskDetail {
  ...
  const base = {
    ...
    latitude: park?.latitude ?? raw.latitude ?? resolveCoordinate(raw.address)?.latitude,
    longitude: park?.longitude ?? raw.longitude ?? resolveCoordinate(raw.address)?.longitude,
...
export async function fetchTaskList(): Promise<TaskListData> {
  const data = mockData as TaskListData
  return {
